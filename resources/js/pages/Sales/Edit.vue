...
<AuthenticatedLayout>
    <template #header>
        <h2 class="text-xl font-semibold">Edit Sale</h2>
    </template>
...
<div class="max-w-6xl mx-auto py-8 space-y-6">
    ...
    <div class="bg-white p-6 rounded shadow">
        ...
        <div v-if="selectedContact" class="mt-3 bg-green-100 p-2 flex justify-between">
            <span>{{ selectedContact.name }}</span>
            <button @click="clearContact">✕</button>
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded shadow grid grid-cols-2 gap-4">
        ...
        <input v-model="form.town_city" placeholder="Town" class="border p-2" />
    ...
    <div class="bg-white p-6 rounded shadow">
        ...
        <div class="flex justify-end gap-2 mb-4">
            <button @click="showProductModal = true" class="bg-green-600 text-white px-3 py-1 rounded">
                + Product
            </button>
                ...
                <tr v-for="(item, i) in form.items" :key="i" class="border-t">
                    ...
                    <td class="p-2">
                        <div class="flex items-center gap-2">
                            <img v-if="item.image" :src="item.image" class="w-10 h-10 rounded" />
                    ...
                    <td><input v-model.number="item.price" type="number" class="border p-1 w-20" /></td>
                    <td><input v-model.number="item.qty" type="number" class="border p-1 w-16" /></td>
                    <td><input v-model.number="item.discount" type="number" class="border p-1 w-20" /></td>
